...
#ifndef HHFFT_COMMON_SSE2_D
#define HHFFT_COMMON_SSE2_D
...
template<size_t radix, bool forward> inline __attribute__((always_inline)) void multiply_coeff_D2S(const ComplexD2S *x_in, ComplexD2S *x_out)
{
    ...
    if (radix == 7)
    {
        ...
        {
            sums[i] = x_in[i+1] + x_in[radix - i - 1];
            diffs[i] = mul_i_D2S(x_in[radix - i - 1] - x_in[i+1]);
        }
        ...
        for (size_t i = 0; i < 3; i++)
        {
            ComplexD2S x = broadcast64_D2S(0);
            for (size_t j = 0; j < 3; j++)
            {
                ComplexD2S coeff = broadcast64_D2S(coeff_cos[3*i + j]);
                x = x + coeff*sums[j];
            }
        ...
        for (size_t i = 0; i < 3; i++)
        {
            ...
            for (size_t j = 0; j < 3; j++)
            {
                ComplexD2S coeff = broadcast64_D2S(coeff_sin[3*i + j]);
                x = x + coeff*diffs[j];
            }
            if (forward)
            {
