...
template<bool scale> void fft_1d_complex_reorder_sse2_f(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info)
{
...
template<RadixType radix_type>
    inline __attribute__((always_inline)) void fft_1d_complex_sse2_f_internal_stride1(const float *data_in, float *data_out, float *data_raders, const hhfft::RadersF &raders, size_t repeats)
{
    size_t i = 0;
...
template<RadixType radix_type, bool forward>
    inline __attribute__((always_inline)) void fft_1d_complex_sse2_f_internal_stride1_reorder(const float *data_in, float *data_out, size_t repeats, const hhfft::StepInfo<float> &step_info)
{
    size_t i = 0;
    uint32_t *reorder_table = step_info.reorder_table;
    float k = step_info.norm_factor;
...
template<RadixType radix_type>
    inline __attribute__((always_inline)) void fft_1d_complex_sse2_f_internal_striden_reorder_forward(const float *data_in, float *data_out, size_t stride, const hhfft::StepInfo<float> &step_info)
{
    const hhfft::RadersF &raders = *step_info.raders;
    size_t radix = get_actual_radix<radix_type>(raders);
    uint32_t *reorder_table = step_info.reorder_table;
...
template<RadixType radix_type>
    inline __attribute__((always_inline)) void fft_1d_complex_sse2_f_internal_striden_reorder_inverse(const float *data_in, float *data_out, size_t stride, const hhfft::StepInfo<float> &step_info)
{    
    const hhfft::RadersF &raders = *step_info.raders;
    size_t radix = get_actual_radix<radix_type>(raders);
    uint32_t *reorder_table = step_info.reorder_table;
    ComplexF2 k = broadcast32_F2(step_info.norm_factor);
    size_t reorder_table_size = step_info.reorder_table_size;
    ...
    for (i = 0; i + 1 < stride - 1; i+=2)
    {
        ...
        for (size_t j = 0; j < radix; j++)
        {
            ...
            ComplexF2 x = load_F2(data_in + 2*i2);
            set_value_F2<radix_type>(x_temp_in, data_raders, radix - j - 1, raders, x);
        }
        ...
        for (size_t j = 0; j < radix; j++)
        {
            ComplexF2 x = k*get_value_F2<radix_type>(x_temp_out, data_raders, radix - j - 1, raders);
            size_t i2 = j*stride + stride - i - 2;
...
template<RadixType radix_type, SizeType stride_type>
    inline __attribute__((always_inline)) void fft_1d_complex_sse2_f_internal(const float *data_in, float *data_out, float *data_raders, const hhfft::RadersF &raders, size_t stride)
{
    size_t k = 0;
    size_t radix = get_actual_radix<radix_type>(raders);
...
template<RadixType radix_type, SizeType stride_type>
    inline __attribute__((always_inline)) void fft_1d_complex_twiddle_dit_sse2_f_internal(const float *data_in, float *data_out, const float *twiddle_factors, float *data_raders, const hhfft::RadersF &raders, size_t stride)
{
    size_t k = 0;
    size_t radix = get_actual_radix<radix_type>(raders);
    ...
    void fft_1d_complex_sse2_f(const float *data_in, float *data_out, const hhfft::StepInfo<float> &step_info)
{
    ...
    if (stride_type == SizeType::Size1)
    {
        ...
        fft_1d_complex_sse2_f_internal_stride1<radix_type>(data_in, data_out, data_raders, raders, repeats);
    } else
    {
        // Amount of Raders memory needed depends on stride
        if (stride > 1)
        ...
        {
            fft_1d_complex_sse2_f_internal<radix_type,stride_type>
    ...
    void fft_1d_complex_reorder2_sse2_f(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info)
{
    ...
    if (stride_type == SizeType::Size1)
        fft_1d_complex_sse2_f_internal_stride1_reorder<radix_type, forward>(data_in, data_out, repeats, step_info);
    else
    {
        // If stride > 1, first fft, then reorder. This is more efficient way.
        if (forward)
            fft_1d_complex_sse2_f_internal_striden_reorder_forward<radix_type>(data_in, data_out, stride, step_info);
        else
            fft_1d_complex_sse2_f_internal_striden_reorder_inverse<radix_type>(data_in, data_out, stride, step_info);
    }
}
    ...
    void fft_1d_complex_twiddle_dit_sse2_f(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info)
{
    ...
    for (size_t i = 0; i < repeats; i++)
    {
        fft_1d_complex_twiddle_dit_sse2_f_internal<radix_type,stride_type>
...
void fft_1d_complex_convolution_sse2_f(const float *data_in0, const float *data_in1, float *data_out, size_t n)
{
    size_t i;
...
template void fft_1d_complex_reorder_sse2_f<false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder_sse2_f<true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);

template void fft_1d_complex_reorder2_sse2_f<Raders, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Raders, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix2, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix2, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix3, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix3, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix4, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix4, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix5, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix5, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix6, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix6, SizeType::Size1, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix7, SizeType::Size1, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_reorder2_sse2_f<Radix2, SizeType::SizeN, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix3, SizeType::SizeN, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix3, SizeType::SizeN, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix4, SizeType::SizeN, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix4, SizeType::SizeN, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix5, SizeType::SizeN, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix5, SizeType::SizeN, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix6, SizeType::SizeN, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix6, SizeType::SizeN, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_reorder2_sse2_f<Radix7, SizeType::SizeN, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_sse2_f<Radix3, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_sse2_f<Radix4, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_sse2_f<Radix5, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_sse2_f<Radix4, SizeType::SizeN>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_sse2_f<Radix5, SizeType::SizeN>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_twiddle_dit_sse2_f<Radix3, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_twiddle_dit_sse2_f<Radix4, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_twiddle_dit_sse2_f<Radix5, SizeType::Size1>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_twiddle_dit_sse2_f<Radix4, SizeType::SizeN>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_twiddle_dit_sse2_f<Radix5, SizeType::SizeN>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_1level_sse2_f<3, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_1level_sse2_f<4, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_1level_sse2_f<5, false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_1level_sse2_f<4, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_1level_sse2_f<5, true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
...
template void fft_1d_complex_2level_sse2_f<2, 5, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<2, 5, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<2, 6, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<2, 6, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<2, 7, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<2, 7, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<3, 5, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<3, 5, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<4, 4, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<4, 4, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<3, 6, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<3, 6, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
template void fft_1d_complex_2level_sse2_f<4, 5, true>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);
...
template void fft_1d_complex_2level_sse2_f<8, 8, false>(const float *data_in, float *data_out, const hhfft::StepInfo<float> &);

template void fft_1d_complex_1level_raders_sse2_f<true>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
template void fft_1d_complex_1level_raders_sse2_f<false>(const float *data_in, float *data_out,const hhfft::StepInfo<float> &step_info);
