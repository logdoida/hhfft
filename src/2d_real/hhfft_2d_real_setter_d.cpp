...
template<size_t radix> void set_instruction_set_2d_real_last_column_d(StepInfoD &step_info, hhfft::InstructionSet instruction_set)
{
    ...
    if (instruction_set == hhfft::InstructionSet::avx)
    {
        step_info.step_function = fft_2d_real_reorder_last_column_avx_d<radix>;
    }
#endif

    if (instruction_set == hhfft::InstructionSet::sse2)
    {
        step_info.step_function = fft_2d_real_reorder_last_column_sse2_d<radix>;
    }

    if (instruction_set == hhfft::InstructionSet::none)
    {
        step_info.step_function = fft_2d_real_reorder_last_column_plain_d<radix>;
    }
}


void set_radix_2d_real_last_column_d(StepInfoD &step_info, hhfft::InstructionSet instruction_set)
{
    size_t radix = step_info.radix;

    if (radix == 2)
    {
        set_instruction_set_2d_real_last_column_d<2>(step_info, instruction_set);
    } if (radix == 3)
    {
        set_instruction_set_2d_real_last_column_d<3>(step_info, instruction_set);
    } if (radix == 4)
    {
        set_instruction_set_2d_real_last_column_d<4>(step_info, instruction_set);
    } if (radix == 5)
    {
        set_instruction_set_2d_real_last_column_d<5>(step_info, instruction_set);
    } if (radix == 7)
    {
        set_instruction_set_2d_real_last_column_d<7>(step_info, instruction_set);
    } if (radix == 8)
    {
       set_instruction_set_2d_real_last_column_d<8>(step_info, instruction_set);
    }
}

// Used for setting the first step for last column where reordering and first fft are combined
void hhfft::HHFFT_2D_Real_D_set_function_last_column(StepInfoD &step_info, hhfft::InstructionSet instruction_set)
{
    step_info.step_function = nullptr;

    if (step_info.radix != 1 && (step_info.reorder_table != nullptr))
    {
        set_radix_2d_real_last_column_d(step_info, instruction_set);
